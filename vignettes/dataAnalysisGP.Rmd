...
an heterogeneous random noise Poisson-like), and medium-scale
oscillations around the exponential decay 
(model inadequacy; see Fig. \@ref{fig:plotMono}). 
...
(\#eq:modExp)
\end{equation}  
is first fitted to the data by maximization of the posterior pdf (MAP).
The likelihood is
\begin{equation}
...
(\#eq:6)
\end{equation}
$\delta l(.)$, _the lodulation function_, is a Gaussian Process (GP) 
...
(\#eq:9)
\end{equation}
whete $\lambda_r$ is chosen _a priori_. I defines the scale of 
...
ov variations of the mono-exponential residuals [@Pernot2017b], 
...
$\sigma$. The prior on $\sigma$ is a normal distribution, centered on 1, 
